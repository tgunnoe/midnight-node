...
let
  ...
  #
  # Extended to support packageId-based lookup for cases where same-version
  # crates from different sources need different extern names.
  mkRustcDepArgs =
    ...
    lib.concatMapStringsSep " " (
      ...
      let
        ...
        # Find a matching rename, preferring packageId-based match over version-only match.
        # This handles the case where multiple crates have the same name and version
        # but come from different git sources (e.g., ledger-7.0.1 vs ledger-8.0.0-rc.2).
        findRename =
          choices:
          let
            # Try packageId match first (most specific)
            packageIdMatch =
              if dep ? packageId && dep.packageId != null then
                lib.findFirst (choice: (choice ? packageId) && choice.packageId == dep.packageId) null choices
              else null;
            # Fall back to version match
            versionMatch =
              lib.findFirst (choice: (!(choice ? version) || choice.version == dep.version or "")) {
                rename = extern;
              } choices;
          in
          if packageIdMatch != null then packageIdMatch else versionMatch;
        name =
          if lib.hasAttr dep.crateName crateRenames then
            let
              choices = crateRenames.${dep.crateName};
            in
            normalizeName (if builtins.isList choices then (findRename choices).rename else choices)
          else
            extern;
  ...
  (
    ...
    stdenv.mkDerivation (
      rec {
        ...
        # Expose cargo as CARGO env var for proc-macro-crate resolution.
        # proc-macro-crate (used by parity-scale-codec-derive's Encode/Decode)
        # needs to find cargo at compile time to read Cargo.toml metadata.
        ...
        buildInputs =
          ...
          ++ buildInputs_;
        # Preserve packageId attribute for crate2nix packageId-based rename lookups
        dependencies = map (dep: (lib.getLib dep) // { packageId = dep.packageId or null; }) dependencies_;
        buildDependencies = map (dep: (lib.getLib dep) // { packageId = dep.packageId or null; }) buildDependencies_;
        ...
        metadata =
          let
            ...
            # Use packageId if available to disambiguate crates with same name/version
            # from different git sources (e.g., ledger-7.0.1 vs ledger-8.0.0-rc.2)
            uniqueCrateId =
              if crate ? packageId && crate.packageId != null then crate.packageId
              else crateName + "-" + crateVersion;
